...
require_once('BaseList.php');
	class OpticalDriveList extends BaseList {
        ...
        public function getAllFromDatabase() {
            ...
            if($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    $od = new OpticalDrive($row['id'], $row['name'], $row['vendor'], $row['price'], $row['catname'], 
						$this->getOpticalDrivePropertiesById($row['id']));
                    array_push($this->dataArray, $od);
                }
            } else echo "0 results";
        }
		public function getAllFromDatabaseBySearchCriteria($search) {
			$sql = "SELECT `opticaldrive`.*,`category`.name catname FROM `opticaldrive` 
				INNER JOIN `category` ON `opticaldrive`.category_id=`category`.id  
				WHERE LOWER(`opticaldrive`.name) LIKE LOWER('%".$search."%')
				OR LOWER(vendor) LIKE LOWER('%".$search."%')
		...
		public function deleteFromDatabase($id) {
            ...
            $stmt->bind_param("s", $id);
            $stmt->execute();
			$stmt = $this->conn->prepare("SET FOREIGN_KEY_CHECKS=1;");
            $stmt->execute();
		}
